...
export const onRequestPost = async ({ env, request }) => {
  ...
  if (!discordId) return jsonResponse({ error: 'Discord ID is required' }, { status: 400 });
  ...
  try {
    ...
    if (/unique/i.test(msg) && /staff_members/i.test(msg)) {
      return jsonResponse({ error: 'That Discord ID is already a staff member' }, { status: 409 });
    }
    if (/unique/i.test(msg) && /discord_id/i.test(msg)) {
      return jsonResponse({ error: 'That Discord ID is already a staff member' }, { status: 409 });
    }
    return jsonResponse({ error: 'Failed to create staff member' }, { status: 500 });
  }
