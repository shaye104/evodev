...
export const onRequestGet = async ({ env, request, params }) => {
  ...
  let reads = { user: null, staff: null };
  try {
    await ensureTicketReadsSchema(env);
    const staffRead = await env.DB.prepare(
      `
      SELECT last_read_message_id, last_read_at
      FROM ticket_reads
      WHERE ticket_id = ? AND reader_type = 'staff' AND reader_id = ?
      LIMIT 1
      `
    )
      .bind(ticket.id, staff.id)
      .first();
    const userRead = ticket.creator_user_id
      ? await env.DB.prepare(
          `
          SELECT last_read_message_id, last_read_at
          FROM ticket_reads
          WHERE ticket_id = ? AND reader_type = 'user' AND reader_id = ?
          LIMIT 1
          `
        )
          .bind(ticket.id, ticket.creator_user_id)
          .first()
      : null;
    reads = { user: userRead || null, staff: staffRead || null };
  } catch {
    reads = { user: null, staff: null };
  }
