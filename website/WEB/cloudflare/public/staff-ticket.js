...
const renderTicket = (payload) => {
  ...
  thread.innerHTML = '';
  const userLastReadId = Number(payload?.reads?.user?.last_read_message_id || 0) || 0;
  const lastStaffMsgId = (() => {
    for (let i = (messages || []).length - 1; i >= 0; i -= 1) {
      const m = messages[i];
      if (m && m.author_type === 'staff') return Number(m.id || 0) || null;
    }
    return null;
  })();
  messages.forEach((msg) => {
    ...
    if (lastStaffMsgId && Number(msg.id || 0) === lastStaffMsgId) {
      const receipt = document.createElement('div');
      receipt.className = 'read-receipt';
      receipt.textContent = userLastReadId && userLastReadId >= lastStaffMsgId ? 'Seen by user' : 'Not seen by user yet';
...
const fetchTicket = async () => {
  ...
  fetch(`/api/staff/tickets/${id}/read`, { method: 'POST' }).catch(() => null);
};
...
const handleRequestResponse = async () => {
  ...
  if (!id || !btn) return;
  btn.disabled = true;
  const res = await fetch(`/api/staff/tickets/${id}/request-response`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({}),
  });
  ...
  if (!res.ok) {
    if (res.status === 429) {
      const s = Number(data.retry_after_seconds || 0) || 0;
      const m = Math.max(1, Math.ceil(s / 60));
      alert(`You can only request a response once every 30 minutes.\nTry again in ~${m} minute(s).`);
      return;
    }
    alert(data.error || 'Failed to request response');
    return;
  }
  alert('Response requested. The user has been pinged on Discord.');
};
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const requestResponseBtn = document.querySelector('[data-request-response]');
  if (requestResponseBtn) requestResponseBtn.addEventListener('click', handleRequestResponse);
  const closeBtn = document.querySelector('[data-close-open]');
  ...
  if (requestResponseBtn) requestResponseBtn.disabled = !canRequestResponse;
