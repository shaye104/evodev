...
function createSupportRouter({ discord }) {
  ...
  router.get('/admin/integrations', ensureAdmin, async (req, res) => {
    const settings = await supportService.getAppSettings([
      'PAYHIP_API_KEY',
      'DISCORD_WEBHOOK_URL',
    ]);
  ...
  router.post('/admin/integrations', ensureAdmin, async (req, res) => {
    await supportService.setAppSetting('PAYHIP_API_KEY', req.body.payhip_api_key);
    await supportService.setAppSetting(
      'DISCORD_WEBHOOK_URL',
      req.body.discord_webhook_url
    );
    await supportService.logAudit({
      actor_user_id: req.user.id,
      actor_discord_id: req.user.discord_id,
      ...
      entity_id: 'payhip',
      metadata: {
        payhip_api_key_set: Boolean(String(req.body.payhip_api_key || '').trim()),
